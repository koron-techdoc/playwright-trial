...
## あとからわかった、ここまでの試行の問題点

opencode を使った試行に以下の問題点があった

-   ローカルLLMを使っていたつもりが Big Pickle (OpenCode Zenでホストされる期間限定フリーのクラウドモデル) で試行していたケースがあった
-   サブエージェントを使わず、メインエージェントで実行していた
    -   `@playwright-test-planner テストを計画して` といったようにやる必要があったかも
-   環境変数 `SHELL` が未設定であったため、余計な負荷をかけていた
-   ローカルLLMで、コンテキストが増えた際の、pre-fill時間の増大が大きい
